...
describe('Translation System E2E Tests', () => {
  ...
  const testConfig: Required<TsdConfig> & { enableHMR: boolean } = {
    defaultLocale: 'en',
  ...
  beforeAll(async () => {
    ...
    // Mock LibreTranslate client
    mockTranslate = jest.fn().mockImplementation(async (...args: any[]) => {
    ...
    const MockedLibreTranslateClient = LibreTranslateClient as jest.MockedClass<typeof LibreTranslateClient>;
    MockedLibreTranslateClient.mockImplementation(() => ({
  ...
  describe('Translation Manager with Monads', () => {
    ...
    it('should get translation by key using monad', async () => {
      ...
      const translationMonad = manager.getTranslation(key as string, 'es');
  ...
  describe('Cache Persistence', () => {
    it('should save and load cache', async () => {
      ...
      const newManager = createTranslationManager(testConfig as Required<TsdConfig>);
  ...
  describe('Error Scenarios', () => {
    it('should handle network errors', async () => {
      (mockTranslate as any).mockRejectedValueOnce(new Error('Network error'));
    ...
    it('should handle concurrent translations of same text', async () => {
      ...
      // Should translate multiple times in this test setup (no caching in mock)
      // In a real implementation with proper caching, this would be 1
