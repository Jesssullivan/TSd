...
// Mock ViteDevServer interface for testing
...
// Mock the grpc-web-server module to avoid import.meta issues
jest.mock('../../src/vite/grpc-web-server', () => ({
  createGrpcWebServer: jest.fn(() => ({
...
describe('gRPC Integration E2E Tests', () => {
  ...
  beforeAll(() => {
    ...
    mockTranslationManager = {
      getOrCreateTranslation: jest.fn().mockImplementation((...args: any[]) => {
      ...
      addTranslationRequest: jest.fn().mockImplementation((...args: any[]) => {
        const [text, native] = args;
        return {
          value: () => 'mock-key-' + text,
  ...
  describe('gRPC Server Setup', () => {
    it('should create gRPC server', () => {
      const { createGrpcWebServer } = require('../../src/vite/grpc-web-server');
      const grpcServer = createGrpcWebServer(mockServer as ViteDevServer, mockTranslationManager);
      expect(createGrpcWebServer).toHaveBeenCalledWith(mockServer, mockTranslationManager);
