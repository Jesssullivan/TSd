  ...
  return {
    map<U>(fn: (value: T) => U): TranslationMonad<U> {
      ...
      try {
        ...
        // Only log errors if not in test environment or if it's an unexpected error
        if (process.env['NODE_ENV'] !== 'test' || (error as Error).message?.includes('Unexpected')) {
...
export function failedTranslation<T>(error?: Error): TranslationMonad<T> {
  if (error) {
    // Only log errors if not in test environment or if it's an unexpected error
    if (process.env['NODE_ENV'] !== 'test' || error.message?.includes('Unexpected')) {
