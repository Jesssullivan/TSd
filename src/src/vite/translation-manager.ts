...
import { validateTranslationInput, needsTranslation, normalizeForKey } from '../lib/input-validation.js';
...
export function createTranslationManager(config: Required<TsdConfig>) {
  ...
  function generateKey(text: string, native: Locale): string {
    // Use normalized text for consistent key generation
    const normalizedText = normalizeForKey(text);
    const hash = crypto.createHash('sha256');
    hash.update(`${normalizedText}:${native}`);
  ...
  function getOrCreateTranslation(
    ...
    if (!needsTranslation(validation.sanitizedText)) {
      return createTranslationMonad(validation.sanitizedText);
    }
  ...
  function addTranslationRequest(text: string, native: Locale): TranslationMonad<string> {
    ...
    if (!needsTranslation(validation.sanitizedText)) {
      // For empty/whitespace strings, still generate a key but don't translate
      return createTranslationMonad(generateKey(validation.sanitizedText, native));
    }
